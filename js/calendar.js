...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    showHoverInfo: function (obj) { // hover 时显示当天信息
      var _dateStr = $(obj).attr('data');//号

      //calendar 宽 + padding 值
      var calendar_wrapW = $("#calendar").width()+20;
      //calendar 距离top left 多少定位
      var calendar_wrapT =  $('#calendar').offset().top;
      var calendar_wrapL =  $('#calendar').offset().left;

      var triangle = $('.triangle')[0];

      //hover 时 item 距离top  left 值
      var offset_t = $(obj).offset().top - calendar_wrapT - ((this.$calendar_today.height() - $(obj).height()) / 2);
      var offset_l = $(obj).offset().left - calendar_wrapL + $(obj).width();

      //console.log(offset_l)
      //显示最大宽度
      var max_offset_l = offset_l +  118;
      //console.log(max_offset_l,calendar_wrapW)
      ...
      //hover 当日信息 显示最大范围
      if( max_offset_l>calendar_wrapW ){
          this.$calendar_today
              .css({left: offset_l - 120, top: offset_t})
              .stop()
              .animate({left: offset_l - 160 ,top: offset_t, opacity: 1});
        triangle.style.left = 100 +"px";
    ...
    showCalendar: function () { // 输入数据并显示
      ...
      setInterval(function(){
        var now_T = new Date();
        var newT = changT(now_T.getHours()) + ":" + changT(now_T.getMinutes()) + ":" + changT(now_T.getSeconds());
        var now_time = $(".now_time").text(newT);
        function changT(num){
          if(num<10){
            return '0'+num;
          }
    ...
    renderDOM: function () { // 渲染DOM
      ...
      var _titleStr = '<a href="javascrip:;" class="now_time"></a>'+
